    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <header className="sticky top-0 z-50 border-b border-border bg-background/95 backdrop-blur-sm">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <Link href="/auth">
            <button className="inline-flex items-center bg-foreground text-background text-sm font-medium px-5 py-2 rounded-lg shadow-sm hover:shadow-md hover:-translate-y-px active:translate-y-0 transition-all duration-150 cursor-pointer">
              Get Started
            </button>
          </Link>
        </div>
      ...
      <section className="max-w-6xl mx-auto px-6 pt-24 pb-20 text-center">
        <div className="inline-flex items-center gap-2 border border-border rounded-full px-4 py-1.5 text-xs text-muted-foreground mb-8 select-none">
        ...
        <div className="flex items-center justify-center gap-3 mt-10 flex-wrap">
          <Link href="/auth">
            <button className="inline-flex items-center gap-2 bg-foreground text-background text-sm font-medium px-7 py-3 rounded-lg shadow-sm hover:shadow-md hover:-translate-y-px active:translate-y-0 transition-all duration-150 cursor-pointer">
              Start Preparing Free <ArrowRight size={15} />
            </button>
          </Link>
          <a href="#how-it-works">
            <button className="inline-flex items-center gap-1.5 border border-border text-muted-foreground hover:border-input hover:text-foreground bg-background text-sm font-medium px-7 py-3 rounded-lg transition-all duration-150 cursor-pointer">
      ...
      {/* THE CHALLENGE */}
      <section className="border-y border-border py-20 bg-muted">
      ...
      <section className="bg-muted border-y border-border py-24">
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-16">
            ...
            <p className="text-muted-foreground mt-4 max-w-sm mx-auto text-sm leading-relaxed">
...
function FeatureCard({ icon, title, desc, featured = false }) {
  return (
    ...
    >
      <div
        className={`w-10 h-10 rounded-lg flex items-center justify-center mb-6 ${featured ? "bg-background/10 text-background" : "bg-muted text-foreground/80"
          }`}
      >
        {icon}
      </div>
      <h3 className="font-semibold text-base mb-2">{title}</h3>
      <p className={`text-sm leading-relaxed ${featured ? "text-background/70" : "text-muted-foreground"}`}>
        {desc}
      </p>
    </div>
  );
...
function CoverageCard({ icon, title, desc }) {
  return (
    <div className="bg-background border border-border hover:border-input rounded-xl p-8 transition-all duration-200">
...
function Step({ number, title, desc }) {
  return (
    <div className="bg-background p-10">
      <p className="text-5xl font-black text-muted-foreground/20 mb-5 leading-none select-none">{number}</p>
      <h3 className="font-semibold text-base mb-2">{title}</h3>
      <p className="text-sm text-muted-foreground leading-relaxed">{desc}</p>
    </div>
  );
