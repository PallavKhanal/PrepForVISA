    ...
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
      {kpis.map((kpi, idx) => {
        ...
        return (
          <div
            key={idx}
            className="bg-background rounded-xl border border-border hover:border-input p-8 transition-all duration-150"
          >
            <div className="flex items-start justify-between mb-6">
              <span className="text-[11px] font-semibold uppercase tracking-widest text-muted-foreground">
                {kpi.title}
              </span>
              <div className="w-10 h-10 rounded-lg bg-muted text-foreground/80 flex items-center justify-center shrink-0">
                <Icon size={18} />
              </div>
            </div>
            <div className="flex items-baseline gap-0.5">
              ...
              {kpi.suffix && (
                <span className="text-lg font-semibold text-muted-foreground ml-0.5">{kpi.suffix}</span>
              )}
            </div>
            <div className={`flex items-center gap-1 mt-2 text-xs font-semibold ${kpi.trend === "up" ? "text-foreground/80" : "text-muted-foreground"}`}>
              {kpi.trend === "up"
